...
{
	bool ok;
	std::clog << "[sql]" << qPrintable(message) << " ... ";
	...
	if (ok)
	{
		...
		if (query.isSelect())
		{
			...
			std::clog << qPrintable(QString("(return %1 records)").arg(count));
		}
		else
			std::clog << qPrintable(QString("(affected %1 records)").arg(query.numRowsAffected()));
	}
	else
	{
		std::clog << "Fail.";
		std::cerr << "SQL: " << qPrintable(query.lastQuery()) << "."
				  << "Reason: " << qPrintable(query.lastError().text());
	}
	std::clog << std::endl;
	if (!ok)
		throw 1;
...
int main(int argc, char *argv[])
{
	...
	{
		std::clog << " FAIL. query: " << qPrintable(e.lastQuery()) << ". " << "Reason: " << qPrintable(e.lastError().text()) << std::endl;
	}
		...
		while (selectSettings.next())
		{
			...
			if (performSql("get active orders count", selectCurrentRoundActiveOrdersCount, param))
				if (selectCurrentRoundActiveOrdersCount.next())
				{
					...
					std::clog << "active orders count: " << count  << std::endl;
				}
