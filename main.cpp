...
int main(int argc, char *argv[])
{
    ...
    QSqlQuery selectCurrentOrders(db);
    if (!selectCurrentOrders.prepare("SELECT order_id from orders where status=-1 and settings_id=:settings_id"))
        qWarning() << selectCurrentOrders.lastError().text();
    ...
    if (!selectOrdersFromPrevRound.prepare("SELECT order_id from orders where backed_up=1 and status<1 and settings_id=:settings_id"))
        qWarning() << selectOrdersFromPrevRound.lastError().text();
...
//				qDebug() << QString("we can spend %1 %3, and get %2 %4").arg(spent).arg(bought).arg(p.currency()).arg(p.goods());
        ...
        while (selectSettings.next())
        {
            ...
            qDebug() << "check if any orders have status changed";
            selectCurrentOrders.bindValue(":settings_id", settings_id);
            if (!selectCurrentOrders.exec())
                qWarning() << selectCurrentOrders.lastQuery() << selectCurrentOrders.lastError().text();
            else
                while (selectCurrentOrders.next())
                {
                    Order::Id order_id = selectCurrentOrders.value(0).toInt();
