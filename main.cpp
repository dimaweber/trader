...
QMap<QString, QString> Trade::extraQueryParams()
{
	...
	params["amount"] = QString::number(amount, 'f', Pairs::ref(pair).decimal_places);
...
//	QSqlQuery selectChanged(db);
//	if (!selectChanged.prepare("SELECT order_id from orders where status=1 and backed_up=0"))
//		qWarning() << "fail to prepare selectChanged statement: " << selectChanged.lastError().text();
	...
	if (!selectSettings.prepare("SELECT profit, comission, first_step, martingale, dep, coverage, count, currency, goods from settings where id=:settings_id"))
		qWarning() << "Fail to prepare select settings statement:" << selectSettings.lastError().text();

	QSqlQuery selectCurrentRound(db);
	if (!selectCurrentRound.prepare("SELECT count(*) from orders where settings_id=:settings_id and backed_up=0"))
	...
	while (1)
	{
		qDebug() << "next round start";

		pticker.performQuery();
		selectSettings.bindValue(":settings_id", settings_id);
		if (!selectSettings.exec())
			qWarning() << "Fail to retrieve settings: " << selectSettings.lastError().text();
		else
		{
			if (selectSettings.next())
			{
				profit = selectSettings.value(0).toDouble();
		...
		if (!round_in_progress)
		{
			...
			double u = qMax(qMin(funds[currency], dep) / execute_rate / sum, pair.min_amount);
			...
			qDebug() << "round already in progress";
		...
		while (!round_done)
		{
			...
			else
			{
				while(sql.next())
				{
					...
					double amount_gain = sql.value(1).toDouble();
					double money_spent = sql.value(2).toDouble();
					...
					if (sell.performQuery())
					{
						...
						insertOrder.bindValue(":start_amount", sell.received);
