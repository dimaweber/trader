...
class TickerTest : public QObject
{
    ...
    QSqlQuery query;
public:
    TickerTest(QSqlDatabase& db):query(db)
    {}
private slots:
    void tst_emptyList()
    {
        ...
        QVariantMap responce = getTickerResponce(query, parser);
...
class InfoTest : public QObject
{
    ...
    QSqlQuery query;
public:
    InfoTest(QSqlDatabase& db):query(db){}
private slots:
    void tst()
    {
        QVariantMap responce = getInfoResponce(query);
...
class DepthTest : public QObject
{
    ...
    QSqlQuery query;
public:
    DepthTest(QSqlDatabase& db):query(db)
    {}
private slots:
    void tst_emptyList()
    {
        ...
        // Out:  {"success":0, "error":"Empty pair list"}
        QueryParser parser("http://localhost:81/api/3/depth");
        QVariantMap responce = getDepthResponce(query, parser);
        QVERIFY(!responce.isEmpty());
        QVERIFY(responce.contains("success"));
        QCOMPARE(responce["success"].toInt(), 0);
        QVERIFY(responce.contains("error"));
