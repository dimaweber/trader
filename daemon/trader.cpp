...
Trader::Trader(QSettings& settings, Database& database, bool& exit_asked, QObject* parent)
    : QObject(parent), settings(settings), database(database), exit_asked(exit_asked), pdepth(1000)
{
...
void Trader::process()
{
    emit statusChanged(StatusServer::Running);
    ...
    try
    {
        ...
        {
            throw std::runtime_error("fail to get settings list");
        }
        ...
        runTimer.setInterval(1000 * 10);
    }
    catch(const QSqlQuery& e)
    {
        std::cerr << "Fail sql query: " << e.executedQuery() << " [" << e.lastError().number() << "] " << e.lastError().text() << std::endl;
        emit statusChanged(StatusServer::Db_Issue);
        database.init();
        runTimer.setInterval(1000 * 30);
    }
    catch (const BtcTradeApi::Api& e)
    {
        emit statusChanged(StatusServer::Btc_Issue);
        std::cerr << "Fail http query: " << e.error() << std::endl;
        runTimer.setInterval(1000 * 60);
    }
    catch (const HttpError& e)
    {
        emit statusChanged(StatusServer::Http_Issue);
        std::cerr << "Http error: " << e.what() << std::endl;
        runTimer.setInterval(1000 * 60);
    }
    catch (const std::runtime_error& e)
    {
        emit statusChanged(StatusServer::Unknown_Issue);
    ...
    if (exit_asked)
    {
        emit statusChanged(StatusServer::Done);
        emit done();
    }
    else
    {
