...
select o.order_id from orders o left join rounds r on r.round_id=o.round_id where r.reason='done' and ( o.status < 1 or o.status > 3);

-- canceled orders with amount and start_amount mismatch
select o.order_id from orders o where o.status=2 and o.amount <> o.start_amount;

-- complete orders with non-zero amount
select o.order_id from orders o where o.status=1 and o.amount <> 0;
