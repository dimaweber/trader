--   check database version is 2.3
select * from (select major,minor from version order by id desc limit 1) A where major<>2 or minor <> 3;
...
select r.round_id from rounds r where (r.end_time is not null and r.reason = 'active') or (r.end_time is null and r.reason in('archive', 'done'));
--FR rounds with no done / partially done orders (might be false positive)
...
--FR rounds with no orders at all (might temporary be false positive)
select r.round_id from rounds r left join orders o on o.round_id=r.round_id where  o.order_id is null;
--FR rounds with only sell orders, no buy orders
...
--FR done rounds with buy orders only, no sell orders
...
--FR rounds with sell order and zero done or partially done buy orders
select S.round_id from (select round_id, count(*) as cnt from orders where type='sell' group by round_id) S left join (select round_id, count(*) as cnt from orders where type='buy' and status_id in (1, 3, 4) group by round_id) B on B.round_id=S.round_id where B.cnt=0 and S.cnt>0;
--FR rounds with currency spend higher then dep_usage
select round_id from rounds where c_out - dep_usage > .01;
--   settings with negative deposit
...
--   no archive round for enabled settings
select id, count(*) cnt from rounds r left join settings s on s.id=r.settings_id where s.enabled=1 and r.reason='archive' group by s.id having cnt <> 1;
